...
\item 第三个是网关（或路由），所有都填192.168.0.1。
\end{itemize}
...
此外，这里顺便设置一下网络接口速度，设置成千兆的，不然可能会自动变成百兆的，降低节点间通信速度。先确定内网网络接口，假如是em2，然后利用root权限执行：

[root]\#  ethtool  -s  em2  autoneg  off  speed 1000  duplex  full

注意，每台计算机都按照上述方式设置一下。
主节点和连接外网的子节点IP设置同上，网络接口速度设置同上。
