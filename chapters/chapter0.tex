...
\section{文献管理、检索与推送}
文献是科研之魂，高效管理文献是科研的第一步——冷轩 \quad 语
\subsection{本地文献组织形式}
最开始，我喜欢用Group或科研主题为单位创建文件夹。后来发现，这种组织形式很不方便，原因有：
\begin{itemize}
\item 随着时间推移和自己的积累，可能有更多的分类或重点不一样了。
\item 最重要的是要平等的看待每一篇文献，发现多年前的Nature、Science 正刊等对现在没有任何参考意义。文献的时效性太短了。
\end{itemize}
所以我觉得文献存储组织形式就按自己文献下载时间年/月来创建文件夹存放。如：Papers/2020/9，存放 自己在2020年9月下载的文献。当然，除了常规的期刊文献外，还有毕业论文等文献，这类文献我习惯分类存放，如图（\ref{fig554}）所示。总体的文献管理则采用专业的文献管理软件，非常推荐Zotero。
\begin{figure}[h!]
\centering
\includegraphics[width=0.8\textwidth]{pictures/papers-organizing.png}
\caption{本地文献组织形式举例}
\label{fig554}
\end{figure}
...
分类区又分为上下两区，上面我习惯跟文献本地文件夹对应起来称为 Collection，下面则是每篇文献的标签汇总称为Tag。
\begin{itemize}
\item 前往官网下载安装相应版本和插件，并注册帐号。
\item 在 Library 创建 Collection，可以与本地文件夹对应也可以不对应，我习惯对应。
\item 选中 Collection 就可以在中间文献区关联本地文献。这样 Zotero 就自动获取文献名、作者、年份等信息。
...
\paragraph{（2）Google浏览器插件——快速文献下载与导入}
通过Google插件，可以一步实现文献下载和Zotero的导入。但是前提的有，1）Zotero是打开着的；2）自己有文献下载的权限，能把文献下载下来，如果不能，则只会保存一个文献信息，需要后期关联。



\paragraph{（3）多平台、多设备同步}
这是我最喜欢的第二功能，我个人主要工作系统是 Ubuntu 系统。但为了实现无纸化办公，我喜欢用微软的 Surface 来阅读文献。而且用 Surface 的笔可以直接在pdf文献上标注，标注也可以同步到主要工作电脑上，非常方便。所以需要Zotero多平台、多设备同步功能。Zotero 自己是有空间同步本地文献的，但是太贵。可以结合国内高质量的坚果云来实现。参考如下：
\begin{itemize}
\item \url{http://help.jianguoyun.com/?p=3168}
\item \url{https://zhuanlan.zhihu.com/p/28325366}
\end{itemize}
也可以使用任意第三方云同步文献夹，然后利用Zotero相对路经关联功能，如图（\ref{fig599}）所示。
\begin{figure}[h!]
\centering
\includegraphics[width=0.6\textwidth]{pictures/zotero-1.png}
\caption{Zotero基本界面}
\label{fig599}
\end{figure}
