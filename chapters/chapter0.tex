...
\chapter{AI助手——你的第二小脑}
\section{基本认识}
AI 助手不替代你想清楚问题，但能少写重复操作、少翻手册。写代码、改 \LaTeX、查报错、整理 git，都可以先问它。
...
\section{Cursor}
Cursor 是带 AI 的代码编辑器，从 VS Code 长出来。打开本地仓库后，可以选中文件或让 Agent 改一整处功能。适合「项目就在眼前、一边看一边改」。

官网与安装：\url{https://cursor.com}
...
\end{verbatim}
Fedora / RHEL 用 dnf 源，见 \url{https://cursor.com/docs} 。也可以从官网下 AppImage。装好后打开项目文件夹，登录账号即可。

\section{Claude Code}
Claude Code 是 Anthropic 的编程代理，主要在终端里用（也有桌面版）。在仓库目录里启动后，可以读代码、改文件、跑 git 和编译命令。没有完整 IDE 界面，更像「在项目里雇一个会用命令行的助手」。

官网：\url{https://code.claude.com/docs}

Linux / macOS / WSL（官方推荐）：
\begin{verbatim}
curl -fsSL https://claude.ai/install.sh | bash
\end{verbatim}
Homebrew：\verb|brew install --cask claude-code|。

装好后在项目目录执行 \verb|claude|，按提示登录 Anthropic 账号。需要 git、能访问外网。
...
Codex CLI 官网：\url{https://developers.openai.com/codex/cli}

Linux / macOS：
\begin{verbatim}
curl -fsSL https://chatgpt.com/codex/install.sh | sh
\end{verbatim}
也可用 \verb|npm install -g @openai/codex| 或 \verb|brew install --cask codex|。

装好后在项目目录执行 \verb|codex|，按提示用 ChatGPT 账号登录。

\section{怎么选}
\begin{itemize}
\item 已经习惯 VS Code、要对着文件改：先装 Cursor。
\item 习惯终端、任务比较整段：Claude Code 或 Codex CLI，选你已有账号的那一家即可。
\item 先讨论、还没打开仓库：用 ChatGPT 网页或桌面应用也行。
\end{itemize}
同一件事用哪一个都能做完。先把一个跑通，比三个都浅装更有用。


\chapter{Git——程序和文档的版本管理}
\section{基本认识}
\begin{itemize}
\item Git 是程序和文档的版本控制软件，有图形界面，也有命令行界面，命令行更简单。
\item GitHub、GitLab、Gitee（码云）是程序托管网络云平台。
\end{itemize}
...
git config --global user.name "xuanleng"

git config --global user.email "lengxuan2008@qq.com"

创建 git 仓库：

mkdir magnetic-filed-visualizion-doc

cd magnetic-filed-visualizion-doc
...
git remote add origin https://gitee.com/nbu-lengxuan/mfv.git
...
\subsection{Github和Gitee同步提交的方法}
ref: \url{https://zhuanlan.zhihu.com/p/143666689}
...
\begin{enumerate}
\item 在项目目录找到.git隐藏文件夹，如果在项目目录找不到.git隐藏文件夹，文件夹右上角打开查看然后找到隐藏的项目并勾选即可

\item 用记事本打开config文件，把Gitee(码云)的仓库地址填进去即可，如下所示
\begin{verbatim}
[core]
...
\item 最后重新push即可，如果Gitee(码云)仓库有过提交，使用此方法第一次提交请使用强行提交，避免报错：\verb| git push -f origin master | 
\end{enumerate}


\subsection{gitee Permission denied问题}
Refs:
\begin{itemize}
\item \url{https://blog.csdn.net/dyy_csdn/article/details/81508809}
\item \url{http://git.mydoc.io/?t=154712}
\end{itemize}
在gitbash中输入命令就可以。
\begin{itemize}
\item 重新生成ssh\\
\verb|ssh-keygen -t rsa -C “xxxxx@xxxxx.com”|

\item 查看你的 public key\\
\verb|cat ~/.ssh/id_rsa.pub|
（以ssh-rsa 开头，以账号的注册邮箱结尾的）

\item 将它添加到码云，添加地址 https://gitee.com/profile/sshkeys

\item 终端输入: \verb|ssh -T git@gitee.com|
\end{itemize}
