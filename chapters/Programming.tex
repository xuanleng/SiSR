...
\section{学习一门编程语言的基本思路}
\begin{itemize}
\item 边用边学，在应用中学习与升级；
\item 手头备一本入门书，翻遍基本语法，有个印象即可，随时查阅；
\item 一个上午完成一门编程语言的入门，就可以开始上手解决自己的问题；
\item 所思即所写，充分搜索网络上自己所需的功能；
\item 程序不通过，注意查报错；
\item 中英文相结合搜索，分别上不同的搜索引擎。
\end{itemize}
一本书在手（了解基本结构），再加上网上搜索资料，就可以开始。
...
\item 注意导入自己所需的库。
\end{itemize}
优点：开源语言，全世界的人都在开发各样的库来满足需求。


\section{开发环境——Anaconda与Spyder}
Anaconda 便于管理 Python 库和环境。
\begin{figure}[h!]
\centering
\includegraphics[width=0.8\textwidth]{pictures/anaconda.jpeg}
\caption{Anaconda 便于管理库}
\label{fig:anaconda}
\end{figure}

Spyder 是常用的科学计算 IDE，界面和 Matlab 很像。
\begin{figure}[h!]
\centering
\includegraphics[width=0.8\textwidth]{pictures/Spyder.png}
\caption{Spyder 界面和 Matlab 非常像}
\label{fig:spyder}
\end{figure}
...
\section{conda环境管理}
\begin{itemize}
\item conda info --envs ：展示当前建立的python环境
\end{itemize}

创建默认环境：conda env create -f environment.yml

\begin{verbatim}
# Scholar Report AI - Conda环境配置
# 为Anaconda用户提供的环境配置文件
...
# 环境位置: ~/anaconda3/envs/scholar-report

# 方法2: 指定自定义目录
# conda env create -f environment.yml --prefix ./envs/scholar-report
# conda env create -f environment.yml --prefix /path/to/custom/location

# 方法3: 在项目目录创建 (不推荐，会污染项目目录)
# conda env create -f environment.yml --prefix ./.conda-env

# ===== 激活环境 =====
# conda activate scholar-report  # 方法1创建的环境
# conda activate ./envs/scholar-report  # 方法2创建的环境

# ===== 删除环境 =====
# conda env remove -n scholar-report  # 方法1创建的环境
# conda env remove --prefix ./envs/scholar-report  # 方法2创建的环境
\end{verbatim}•


激活环境：conda activate scholar-report
