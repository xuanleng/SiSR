...
\begin{enumerate}
\item 这是一份开源协作文档。
\item 这是一份为科研入门（理论计算为主）提供必要基本知识的文档，旨在积累科研中的经验和技巧，让新手快速上手和少走弯路。
...
\item 文档结构：前言章节为固定结构，由文档定位、组织形式、历任主副编、经历分享和推送记录构成。
\begin{enumerate}
\item 经历分享：基于文档定位，这是一个避坑文档，所以鼓励大家分享踩坑、脱坑经历，写在经历分享中。注意：不接受针对具体人物事件的分享。
\item 推送记录：希望大家推送时，写下自己的称谓在推送记录中记录一下。
\end{enumerate}
\end{itemize}
...
\section*{推送记录}
\addcontentsline{toc}{section}{推送记录}
\begin{enumerate}
\item 05-26-2020: xuan更新了前言内容，进一步明确了文档目的和组织形式。
\end{enumerate}
