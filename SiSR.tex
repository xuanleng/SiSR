...
% 宏包（listings 已由 elegantbook 载入）
\usepackage{verbatim}          % \begin{comment}
\usepackage{longtable}
\usepackage{ulem}              % \sout
\usepackage{draftwatermark}
% \includeonly{chapters/chapter0} % 调试单章时取消注释
...
\lfoot{主讲“器”的思维逻辑}
\cfoot{\thepage}
\rfoot{\hyperref[content]{目录}}

% 目录到 subsection；正文编号仍到 subsubsection
\setcounter{tocdepth}{2}
\setcounter{secnumdepth}{3}

% 代码列表：统一样式，语言由各处 [language=...] 指定
\lstdefinestyle{sisr}{
  ...
  numbersep=3mm,
  numbers=left,
  numberstyle=\scriptsize\color{magenta},
  frame=single,
...
\lstdefinestyle{fortran}{
  style=sisr,
  language=Fortran,
  morekeywords={aimag,open,real*8},
}
...
\tikzset{
  gitbox/.style={
    draw=structurecolor, fill=structurecolor!8,
    rounded corners=2pt, align=center,
    minimum width=2.25cm, minimum height=0.95cm,
    inner sep=3pt, font=\small
  },
  gitremote/.style={gitbox, draw=second, fill=second!14},
  gitarr/.style={-{Stealth[length=2mm]}, semithick, structurecolor},
  gitback/.style={-{Stealth[length=2mm]}, semithick, second},
  gitlab/.style={font=\scriptsize, fill=white, inner sep=1.5pt}
}

% AI小脑思维：全书统一的提示框。
% \begin{xiaonao} ... \end{xiaonao}
% \begin{xiaonao}[如何学习git] ... \end{xiaonao}  → 标题为「AI小脑思维之如何学习git」
\definecolor{xiaonao}{RGB}{106,61,154}
\definecolor{xiaonaohi}{RGB}{255,196,64}
\newcommand{\xiaonaoicon}{%
  \begin{tikzpicture}[x=0.9pt,y=0.9pt,line join=round,line cap=round]
    % 大脑，头顶在上；小脑在后下方，用亮色突出
    \fill[white]
      (4.2,7.2)
      .. controls (2.6,9.6) and (2.8,13.4) .. (6.2,15.6)
      .. controls (9.8,17.8) and (15.0,17.6) .. (18.0,15.0)
      .. controls (20.4,12.8) and (20.2,9.8) .. (18.0,8.2)
      .. controls (16.2,6.8) and (14.6,8.4) .. (12.4,7.2)
      .. controls (10.0,5.8) and (6.8,6.4) .. (4.2,7.2) -- cycle;
    \draw[xiaonao!55,line width=0.7pt]
      (7.0,13.6) .. controls (8.2,11.2) and (9.4,11.0) .. (8.6,8.6)
      (11.2,14.6) .. controls (12.6,12.0) and (13.6,11.8) .. (12.4,9.2);
    \fill[xiaonaohi]
      (13.2,8.0)
      .. controls (15.4,4.6) and (21.6,4.2) .. (23.2,7.8)
      .. controls (24.6,11.0) and (21.4,13.8) .. (17.0,13.0)
      .. controls (13.6,12.4) and (11.4,10.4) .. (13.2,8.0) -- cycle;
    \draw[xiaonao,line width=0.85pt]
      (14.8,7.6) .. controls (17.6,6.6) and (20.4,6.8) .. (22.2,8.2)
      (14.4,9.6) .. controls (17.4,8.6) and (20.2,8.8) .. (22.0,10.2)
      (15.0,11.4) .. controls (17.6,10.4) and (19.8,10.6) .. (21.0,11.6);
  \end{tikzpicture}%
}
\newtcolorbox{xiaonao}[1][]{
  enhanced,
  breakable,
  arc=2.4mm,
  boxrule=0.6pt,
  ...
  overlay={
    \begin{scope}
      ...
      \fill[xiaonao]
        (frame.north west) rectangle ([xshift=2.55em]frame.south west);
    \end{scope}
    \node[anchor=center, inner sep=0pt]
      at ([xshift=1.28em]frame.west) {\xiaonaoicon};
  },
  before upper={%
    {\bfseries\color{xiaonao}AI小脑思维\ifstrempty{#1}{}{之#1}}%
    \par\vspace{0.2em}%
  },
}
...
\author{\href{https://xuanleng.me}{冷轩 （第一任）}\\
{\kaishu 项目地址}： \url{https://github.com/xuanleng/SiSR}\\
{\kaishu \LaTeX{} 模板}：\url{https://github.com/ElegantLaTeX/ElegantBook}
}
\date{\today}
...
\begin{itemize}
\item 问过AI了吗？
\item \sout{网上搜索过了吗？} 不需要搜索了，直接问AI
\item 自己思考过了吗？
...
\item 2026-09-24:xinlingren 学习了借助git去辅助我大论文的完成，体会到了人工智能的便捷。我第一次把本地的 master 推送到 Gitee。开始没有成功，后来推送成功，在网页上看到了自己的文件。没有搞清楚流程，花了一些功夫才搞清楚，终于完成了第一次推送更新。
\end{enumerate}
...
为方便展示大家的修改，用 git-latexdiff 比较两个 Git 版本，直接生成带红蓝标注的 PDF，如图（\ref{fig:367}）所示。
它会检出两个修订，用 latexpand 把 \verb|\input|、\verb|\include| 展开成单文件，再交给 latexdiff 编译。不必再手工 latexpand 一次、latexdiff 一次。

请用官方版本（当前 1.7.1，2025-07-23）：
\url{https://gitlab.com/git-latexdiff/git-latexdiff} 。
TeX Live / MiKTeX 里一般已有，也可 \verb|tlmgr install git-latexdiff|。
还需要本机能跑 \verb|xelatex|、\verb|latexdiff| 和 \verb|latexpand|。
...
指定两个版本：
\begin{verbatim}
 git latexdiff --xelatex --main SiSR.tex <旧提交> <新提交>
\end{verbatim}

若展开时遇到注释或 \verb|%**| 一类报错，把参数转给 latexpand：
\begin{verbatim}
 git latexdiff --xelatex --main SiSR.tex --latexpand --keep-comments HEAD^
\end{verbatim}
...
\item 已淘汰的技术（如早期 Gnuplot 笔记、Rocky 8 安装笔记）放在仓库目录 \texttt{archive/} 中，不纳入正文。
\end{itemize}
...
也乘着这次更新，本文档也有些变化，最主要的是一些自己淘汰不用的技术要分离，放到专门的历史档案中（见仓库目录 \texttt{archive/gnuplot}）。这些内容不再纳入正文，以免和现行做法混在一起。



\subsection*{2025.3.15——好久没折腾，这次是自己搭建人工智能本地文献知识库}
\addcontentsline{toc}{subsection}{2025.3.15——好久没折腾，这次是自己搭建人工智能本地文献知识库}
离上次折腾已经过去2年了，通过了首聘期考核，紧张的心终于放下来了，可以喘口气做些长期的项目了。这两年世界的变化也是非常剧烈的，新冠疫情、俄乌战争还有人工智能ChatGPT、DeepSeek。这些巨变对我们的生活和工作影响也是巨大的，我们要作出及时的调整。我开始以为，有了ChatGPT，我们这个文档就不用写了。后来发现，反而是更有助于写这个文档。是的，这里是成熟的技术类文档，大部分东西ChatGPT都给到你，为什么还要写。我觉得，写的性质变了，不是一个查询文档，更多的是一个抛砖引玉的文档，还有一个最重要的功能是帮助自己整理知识，将自己的知识系统化。零碎的知识没法形成规模效应。

2026.9.21 感觉时间过了很久，我一点记忆都没有。不知道是年纪大了，还是每天的活太多了，大脑存不下这么多记忆。不过我可以明确，当时失败了。因为现在又再重新做。

\rightline{——冷轩\qquad}


\subsection*{2026.9.21——AI时代再出发}
\addcontentsline{toc}{subsection}{2026.9.21——AI时代再出发}
这份文档沉寂了很多年，现在要再出发了。 开始以为到了AI时代，不需要这份文档了。 确实，具体的细节不需要了，AI会帮忙处理。但是对于初学者更多的需要一个引导。这就是AI时代本文档的任务。所以，我们文档要大幅删简压缩技术细节。
